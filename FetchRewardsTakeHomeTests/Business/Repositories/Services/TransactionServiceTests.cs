...
public class TransactionServiceTests
{
    ...
    public void TestAddTransactionWithExistingPayer()
    {
        ...
        A.CallTo(() => mockPayerRepo.FindPayerByName("test")).ThrowsAsync(new PayerNotFound("test"));
        ...
        var service = new TransactionService(A.Fake<ILogger<TransactionService>>(), mockPointsRepo, mockPayerRepo);
        service.AddTransaction(new PointsModel("test", 500, DateTime.Now));
    ...
    public async void TestLookUpTransactionNotFoundThrowsException()
    {
        ...
        var id = Guid.NewGuid();
        A.CallTo(() => mockPointsRepo.FindTransaction(id)).Throws(new TransactionNotFound(id));
        
        var service = new TransactionService(A.Fake<ILogger<TransactionService>>(), mockPointsRepo, mockPayerRepo);
        Func<Task> action = async () => { await service.LookupTransaction(id);};
    ...
    public async void TestLookUpTransactionFindsTransaction()
    {
        ...
        var service = new TransactionService(A.Fake<ILogger<TransactionService>>(), mockPointsRepo, mockPayerRepo);
        var returnObject = await service.LookupTransaction(id);
